...
export default function TimePractice() {
  ...
  const chartValues = activeRange?.points.map((point) => point.minutes) ?? [];
  const chartLabels = activeRange?.points.map((point) => point.label) ?? [];
  ...
  const renderRangeGrid = (ranges: TimePracticeRange[], columns: number) => (
  ...
  return (
    <ImageBackground source={bgImage} style={styles.screen} resizeMode="cover">
      <SafeAreaView style={styles.safeArea}>
        <TopHeader title="Time Practice" variant="transparent" onBack={() => router.back()} />
      </SafeAreaView>
      ...
      <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.chartCard}>
          <View style={styles.rangeHeader}>
            <Text style={styles.rangeLabel}>{activeRange?.label}</Text>
          ...
          <Svg width={chartWidth} height={170}>
            ...
            {chartValues.map((value, index) => {
              ...
              const xStep = (chartWidth - padding * 2) / Math.max(chartValues.length - 1, 1);
